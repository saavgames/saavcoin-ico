...
contract SaavCoins is ERC20, admin, MigrationAgent {
	...
	function adminSetMigrationHost(address mhost) onlyOwner {
		migrationHost = mhost;
	}
	 ...
	 */
	function SaavCoins() {
		name = "SAAV M1 Coins";		// Set the name for display purposes
		symbol = "SAAV M1";			// Set the symbol for display purposes
...
/*!	Functionality to support migrations to new upgraded contract
	...
	The migration can be set only once.
 */
contract SaavCoinsMigratory is SaavCoins {
	...
	/*! Setup the address for new contract (to migrate coins to)
		NOTE: The migration agent address can be set only once.
...
contract SaavCoinsICO is SaavCoinsMigratory {
	...
	/*! May need to be changed for migrated contract data
	 */
	function adminSetTotalSupplyForSale(uint256 value) onlyOwner {
		totalSupplyForSale = value;
	}
	...
	function SaavCoinsICO() {
		...
		// can not have pre-coded balances until owner migrate balance
	}
}
